...
Difference imaging or image subtraction is a method developed for detecting variable objects in astronomical studies, in which the difference is measured between two images that are both positionally and photometrically matched. This kind of method is optimal for analyzing variabilities in crowded astronomical images, since the method bypasses the procedure of doing photometry for each individual object and comparing with a catalog, but instead directly measures differential photometry.
...
Although \cite{alard}, \cite{varyingkernel} mitigate the numerical instability, the choice of basis function largely affects the performance and may require sufficient information about the PSFs in both the reference and target images.
\cite{bramich} goes even further; they handle more complicated kernels by using a discrete pixel array instead of a linear combination of basis functions.
...
\cite{optimal} based on the likelihood ratio test and derived a closed form of subtraction image, in which the target and reference images are convolved with each other's PSF.
Instead of solving the convolution kernel, good PSFs estimation of both reference and new images is required.
Apart from the kernel estimation, most of these methods require very precise image registration. 
In \cite{alard} and \cite{varyingkernel}, due to the restriction of the basis functions, the kernel is not able to model complex transformation between the reference and target. 
In \cite{optimal}, the convolution procedure can only match the difference of PSFs. 
Thus even more precise astrometry is required.
...
The choice of the reference image is important for both kernel solving and subtraction.
However no robust algorithm for reference image selection has been proposed so far as mentioned in \cite{reference}.
...
\begin{enumerate}
\item The method capitalizes on a huge set of images, not just a single reference image. 
\item No knowledge of the detailed pointing or PSF of any frame is required. 
\item This method is perfectly good on under-sampled imaging
\end{enumerate}
...
\cite{wei} has already applied the classical difference imaging on this data set and is able to model some microlensing events.
...
\begin{enumerate}
\item All classical difference imaging approaches require precise image registration as a priori step. 
...
\item All classical difference imaging approaches require accurate direct or indirect information about the PSF. 
\cite{imagesub1}, \cite{alard} and \cite{bramich} model the difference of PSFs between images by solving for a convolution kernel.
In \cite{optimal}, both PSFs of the reference and the new images need to be known explicitly.
Any imperfection of the PSF(or difference of PSFs) estimation will lead to subtraction residuals.
\cpmdiff\ avoids PSF estimation, since there is no convolution kernel involved, even implicitly.
...
Apart from these benefits from the method, we still want to emphasize that \cpmdiff\ has its own scope of applicability.
The method requires enough images and pixels to optimize the model, so it can not be used in a situation, where  there are very few images or the size of the image is too small. 
From Section \ref{limits}, we learned that although \cpmdiff\ does not require perfect knowledge of pointing or PSF, it is important to ensure the registration to be good within $\sim$1 FWHM of PSF for best performance.
In addition, PSF variations between frames should also be moderate, so that the correlation between pixels are approximately the same.

Although \cpmdiff\ performs impressively in this paper,  in order to fully exploit the potential of the method, it is still important to find a good set of hyper-parameters and optimize the selection of predictors as mentioned in Section \ref{method}. 
...
However, the selection or ranking of the predictor pixels have been intuitively defined here by either using the closest pixels in space or in brightness.
