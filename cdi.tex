...
Thus the mock space-based data only includes the pointing motion and rotation, but maintains the PSF unchanged.
...
\label{space}
  ...
  Most of the pixel values are around zero, except for the variable source (upper-left corner), which shows that \cpmdiff\ can predict the image data, while detecting the variables. 
}
\end{figure}
...
\begin{figure}[p]
\begin{center}
\includegraphics[width=0.95\textwidth]{move_new}
\end{center}
\caption{
\label{motion_rms}
